	...
	for(i = 0 ; i < MAX_I2S_PAGE*2 ; i ++)
	{
		...
		{ 
			...
			if (I2S_PAGE_SIZE >= I2S_MIN_PAGE_SIZE)
				...
				dma_unmap_single(NULL, i2s_mmap_addr[i], I2S_MIN_PAGE_SIZE, DMA_BIDIRECTIONAL);
...
int i2s_driving_strength_adjust(void)
{
        ...
        MSG("Adjust current's driving strngth\n");
	...
	if(dma_ch==GDMA_I2S_TX0)
        {
                ...
                GdmaI2sTx((u32)(pi2s_config->pMMAPTxBufPtr[pi2s_config->tx_r_idx]), I2S_TX_FIFO_WREG, 0, I2S_PAGE_SIZE, i2s_dma_tx_handler, i2s_dma_tx_unmask_handler);
#else
...
#if defined(CONFIG_I2S_MMAP)
		dma_sync_single_for_device(NULL,  i2s_mmap_addr[pi2s_config->tx_r_idx], I2S_PAGE_SIZE, DMA_TO_DEVICE);
                GdmaI2sTx((u32)(pi2s_config->pMMAPTxBufPtr[pi2s_config->tx_r_idx]), I2S_TX_FIFO_WREG, 1, I2S_PAGE_SIZE, i2s_dma_tx_handler, i2s_dma_tx_unmask_handler);
#else
	...
	if(dma_ch==GDMA_I2S_RX0)
        {
                ...
                GdmaI2sRx(I2S_RX_FIFO_RREG, (u32)(pi2s_config->pMMAPRxBufPtr[pi2s_config->next_p0_idx]), 0, I2S_PAGE_SIZE, i2s_dma_rx_handler, i2s_dma_rx_unmask_handler);
#else
	...
	else
        {
                ...
                GdmaI2sRx(I2S_RX_FIFO_RREG, (u32)(pi2s_config->pMMAPRxBufPtr[pi2s_config->next_p1_idx]), 1, I2S_PAGE_SIZE, i2s_dma_rx_handler, i2s_dma_rx_unmask_handler);
#else
		...
		if(aecFuncP->AECNeEnq){
			aecFuncP->AECNeEnq(0,pi2s_config->pMMAPRxBufPtr[pi2s_config->rx_w_idx],I2S_PAGE_SIZE);
		}
#endif
	return 0;
}
...
#endif
{
	...
	case I2S_SRATE:
		spin_lock_irqsave(&ptri2s_config->lock, flags);
#if defined(CONFIG_I2S_WM8960)||defined(CONFIG_I2S_WM8750)
